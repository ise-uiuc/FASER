...
class RelativeMutationID(object):
    ...
    def __repr__(self):
        return 'MutationID(line="{}", index={}, line_number={}, filename={})'.format(self.line, self.index,
...
#     'numpy': {'amax': ['amin', 'max', 'min'],
#                'amin': ['amax', 'max', 'min'],
#                'diag': ['diagflat', 'tril', 'triu'],
#                'diagflat': ['diag', 'info', 'tril', 'triu'],
#                'empty': ['asfarray', 'ones', 'zeros'],
#                'flip': ['info', 'ptp'],
#                'gcd': ['greater', 'lcm', 'less', 'remainder'],
#                'hsplit': ['resize', 'split', 'vsplit'],
#                'invert': ['iterable', 'ndim'],
#                'lcm': ['gcd', 'greater', 'less', 'remainder'],
#                'max': ['amax', 'amin', 'min'],
#                'mean': ['std', 'var'],
#                'min': ['amax', 'amin', 'max'],
#                'ones': ['asfarray', 'empty', 'zeros'],
#                'outer': ['equal', 'inner', 'multiply'],
#                'packbits': ['ptp', 'size'],
#                'ptp': ['flip', 'packbits', 'stack'],
#                'split': ['hsplit', 'resize'],
#                'std': ['mean', 'var'],
#                'tri': ['arange', 'eye'],
#                'var': ['mean', 'std'],
#                'vdot': ['equal', 'extract', 'greater', 'less'],
#                'vsplit': ['hsplit', 'resize'],
#                'zeros': ['asfarray', 'empty', 'ones']}
# }
...
def library_function_mutation(children, imports, **_):  # Need to replace children

    if not (children[0] and children[0].type == 'name' and children[0].value in imports and imports[children[0].value] in replaceable_functions):
        return

    def replace_function_calls(children, replacing_function):
        children = children[:]
        children[1] = copy_obj(children[1])
        children[1].children = copy_obj(children[1].children)
        children[1].children[1] = copy_obj(children[1].children[1])
        children[1].children[1].value = replacing_function
        return children

    if children[1] and children[1].type == 'trailer' and children[1].children and children[1].children[
        0].value == '.' and \
            children[1].children[1].type == 'name' and children[1].children[1].value in replaceable_functions[
        imports[children[0].value]]:
        # print(children)
        ret_list = []
        for replace_function in replaceable_functions[imports[children[0].value]][children[1].children[1].value]:
            # ret_children = replace_function_calls(deepcopy(children), replace_function)
            ret_children = replace_function_calls(children, replace_function)
            ret_list.append(ret_children)
...
mutations_by_type = {
    ...
    'atom_expr': dict(children=library_function_mutation)
}
...
class Context(object):
    ...
    def exclude_line(self):
        return self.current_line_index in self.pragma_no_mutate_lines or should_exclude(context=self,
                                                                                        config=self.config)
    ...
    def mutation_id_of_current_index(self):
        return RelativeMutationID(filename=self.filename, line=self.current_source_line, index=self.index,
...
def find_import_as(node):
    ret = {}

    for child_node in node.children:
        # print(child_node)
        if child_node.type == 'simple_stmt' and child_node.children and child_node.children[0].type == 'import_name':
            c = child_node.children[0].get_code().strip()
            if re.match("import\s\S+\sas\s\S+", c):  # Import something as s
                library = c.split(" ")[1]
                as_value = c.split(" ")[3]
                ret[as_value] = library

            elif re.match("import\s\S+", c):  # Import something
                library = c.split(" ")[1]
                ret[library] = library
...
def mutate(context):
    ...
    imports = find_import_as(result)
    # print(imports)
    mutate_list_of_nodes(result, context=context, imports=imports)
...
def mutate_node(node, context, imports={}):
    """
    :type context: Context
    """
    ...
    try:
        ...
        if hasattr(node, 'children'):
            mutate_list_of_nodes(node, context=context, imports=imports)
        ...
        for key, value in sorted(mutation.items()):
            ...
            if isinstance(new, list) and not isinstance(old, list):
                # multiple mutations
                new_list = new
            elif isinstance(new, list) and isinstance(new[0], list):
                new_list = new
            else:
                # one mutation
                new_list = [new]
            ...
            for new in reversed(new_list):
                ...
                if new is not None and new != old:
                    ...
                    if context.should_mutate():
                        ...
                        context.performed_mutation_types.append(node.type)
...
def mutate_list_of_nodes(node, context, imports={}):
    """
    :type context: Context
    """
    ...
    for child_node in node.children:
        ...
        mutate_node(child_node, context=context, imports=imports)
